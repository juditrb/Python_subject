...
#generating a class for handling incorrect sequence letters
...
#Main class for all sequence types
class Sequence():
    ...
    def __init__(self, identifier, sequence, alphabet):
        ...
        #checking if letters in the sequence are in the specified alphabet
...
#creating the class for protein sequences
class ProteinSequence(Sequence):
    alphabet = set(protein_letters)
#creating the class for nucleotide sequences
class NucleotideSequence(Sequence):
    #translation depending on the type of sequence (DNA or RNA)
    def translate(self):
...
#Iterator function to analyze FASTA files and yield sequence objects
def FASTA_iterator(filename, class_name):
    #mapping class names to specified sequence classes
    class_map = {
    ...
    with open(filename, 'r') as fd:
        ...
        for chain in chains:
            try: 
                ...
                continue
#defining process_files function to process input files, printing information, and optionally write to an output file      
def process_files(filepath, outputpath=None):
    output_sequences = []
    #printing an error if the filepath doesn't exist
...
#defining Main function to drive command-line arguments
def main():
...
######terminal: python u235935_block2_part4.py DNA.fa
